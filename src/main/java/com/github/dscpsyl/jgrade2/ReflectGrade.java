...
final class ReflectGrade {
  ...
  /**
   * Load a class from the current directory. The loader is not closed as the
   * resource is still necessary for the rest of jGrade2 to uitilize. <p> Method
   * is borrowed from <a
   * href="https://github.com/phf/jb">https://github.com/phf/jb</a>.
   *
  ...
  private static boolean hasGradeAnnotation(Method m) {
    ...
    if (num > 1) {
      System.err.printf(
          "method %s has too many grade annotations, should only have 1",
          m.getName());
    }
