...
/**
 * A class that extends a JUnit {@link TestExecutionListener} to check for unit
 * test methods annotated with the {@link GradedTest} annotation. It builds up a
...
public class GradedTestListener implements TestExecutionListener {
   ...
   */
  public int getNumGradedTests() { return this.gradedTestResults.size(); }
   ...
   */
  public int getNumFailedGradedTests() { return numFailedGradedTests; }
   ...
   */
  public Optional<MethodSource>
  getTestMethodSource(TestIdentifier testIdentifier) {
   ...
   *
   * @param ms of the test that finished. Can be returned from {@link
   *     #getTestMethodSource(TestIdentifier)}
   *
  ...
  public Optional<GradedTest> getGradedTestAnnotation(MethodSource ms) {
    ...
    try {
      ...
      GradedTest gradedTestAnnotation =
          testMethod.getAnnotation(GradedTest.class);
      return Optional.ofNullable(gradedTestAnnotation);

    } catch (PreconditionViolationException
                 pve) { // required for getJavaMethod() to be safe
      return Optional.empty();
    }
  }
   ...
   *
   * <p> Compared to the origional jGrade, this is a combination of
   * <code>testFinished</code> <code>testStarted</code>, and
   * <code>testFailure</code>. It will create the new {@link GradedTestResult}
   * and add it to the list of results after the test has finished. It will also
   * set the correct score for the test.
   *
  ...
  public void executionFinished(TestIdentifier testIdentifier,
    ...
    Optional<GradedTest> gradedTestAnnotations =
        getGradedTestAnnotation(ms.get());
    ...
    GradedTestResult currentGradedTestResult = new GradedTestResult(
        gt.name(), gt.number(), gt.points(), gt.visibility());

    // Check the status of the test and set the score
    if (testExecutionResult.getStatus() ==
        TestExecutionResult.Status.SUCCESSFUL) { // All passed, full points
