...
public class GradescopeJsonFormatter implements OutputFormatter {
   ...
   * @return True if the formatter has a visibility set for standard out, false
   *     otherwise.
   */
  ...
  public void setVisibility(String visibility) throws GradescopeJsonException {
    if (!isValidVisibility(visibility)) {
      throw new GradescopeJsonException(visibility +
  ...
  public void setStdoutVisibility(String visibility)
      throws GradescopeJsonException {
    if (!isValidVisibility(visibility)) {
      throw new GradescopeJsonException(visibility +
                                        " is not a valid visibility");
    }
  ...
  public String format(Grader grader) {
    ...
    try {
      return this.prettyPrint >= 0 ? this.json.toString(this.prettyPrint)
                                   : this.json.toString();
   ...
   * @throws GradescopeJsonException If the grader is not valid.
   */
  private void assemble(Grader grader, JSONObject json)
      throws GradescopeJsonException {
    try {
  ...
  private void validateGrader(Grader grader) {
    if (!(grader.hasScore() || grader.hasGradedTestResults())) {
      throw new GradescopeJsonException(
          "Gradescope Json must have either tests or score set");
    }

    /* The following checks ~should~ all pass because they would have been
     * checked when set. */
