 ...
 * <code> <br>
 *    usage: jgrade<br>
 *      -c,--classname arg            the class containing annotated methods to
 * grade<br> -f,--format output-format     specify output, one of 'json'
 * (default) or 'txt'<br> -h,--help<br>
 *         --no-output                don't produce any output (if user
 * overriding)<br> -o destination                save output to another file (if
 * not specified, prints to standard out)<br>
 *         --pretty-print             pretty-print output (when format is
 * json)<br> -v,--version<br>
 * </code>
 */
public final class JGrade2 {
  ...
  private static Grader initGrader(CommandLine line) {
    ...
    if (line.hasOption(FORMAT_OPT) &&
        !line.getOptionValue(FORMAT_OPT).equals(DEFAULT_FORMAT)) {
      String val = line.getOptionValue(FORMAT_OPT);
      switch (val) {
      case TXT_VAL:
        throw new UnsupportedOperationException(
            "have not implemented textual output");
      default:
    ...
    if (line.hasOption(PP_OPT)) {
      if (formatter == null) {
        throw new IllegalArgumentException(
            "pretty-print without json formatting");
      }
  ...
  private static Object instantiateClass(Class<?> c) {
    try {
      ...
      throw new InternalError(
          "instantiateClass::unreachable statement - system should have exited");
    }
  }
  ...
  private static Class<?> getClassToGrade(String className) {
    try {
      ...
      throw new InternalError(
          "getClassToGrade::unreachable statement - system should have exited");
    }
  }
  ...
  private static Options getOptions() {
    ...
    options.addOption(Option.builder()
                          ...
                          .build());
    options.addOption(Option.builder()
                          .longOpt(NO_OUTPUT_OPT)
                          .desc("don't produce any output (if user overriding)")
                          .build());
    options.addOption(
        Option.builder(OUTPUT_OPT)
            .desc(
                "save output to another file (if not specified, prints to standard out)")
            .hasArg(true)
            .argName(DEST_ARG)
            .build());
    options.addOption(
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 0 || line.hasOption(HELP_OPT)) {
      ...
      fatal("missing required class flag",
            new ParseException("missing required class flag"));
    } else {
