...
public abstract class CLITester {
  ...
  static class ExecutionResult implements CLIResult {
    ...
    private static List<String> splitByLines(String s) {
      return s.isEmpty() ? new ArrayList<>()
                         : Arrays.asList(s.split("[\\r\\n]+"));
    }
  }

  private List<String>
      command; // Subclassing classes can append to this to add arguments
  private ProcessBuilder builder; // Subclassing classes can edit things from
                                  // this like redirectErrorStream
   ...
   */
  public void addCommandLineArg(String s) { this.command.add(s); }
   ...
   * @return The result of the execution of the program.
   */
  protected CLIResult runCommand() { return runCommand(null); }
    ...
    try {
      ...
      if (toWriteIn != null) {
        BufferedWriter writer =
            new BufferedWriter(new OutputStreamWriter(driverStdin));
